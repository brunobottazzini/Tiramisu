...
class TiramisuDeckSetupTest {
    ...
    @Test
    fun `tutorial deck initial deal is b1 s6 c3 c10`() {
        val deck = TiramisuDeckSetup.tutorialDeck()
        assertEquals("b1", deck[0])
        // pile 1 = s6 (spade): neutral card, different suit from the coppe trio
        assertEquals("s6", deck[1])
        assertEquals("c3", deck[2])
        // pile 3 = c10: buried under b2 from the second deal; once b2 goes to the foundation
        // c10 is exposed as the landing spot for the [c7,c5] run move
        assertEquals("c10", deck[3])
    }

    @Test
    fun `tutorial deck stock cards index 4 and 5 share suit for same-suit step`() {
        // After 1st deal: pile 0=EMPTY (b1 auto-moved), pile 1=s6, pile 2=c3, pile 3=c10
        // Stock: c5, c7, d3, b2.
        // After 2nd deal: pile 0=c5, pile 1=[s6,c7], pile 2=[c3,d3], pile 3=[c10,b2]
        // Same-suit step: pile 0 (c5, coppe) → pile 1 (c7, coppe) — strict-valid (5 < 7)
        // Indices 4 and 5 must share the same suit (coppe)
        val deck = TiramisuDeckSetup.tutorialDeck()
        val stockCard0 = deck[4] // lands on pile 0 → c5
        val stockCard1 = deck[5] // lands on pile 1 → c7
    ...
    @Test
    fun `tutorial deck pile 3 hides c10 under b2 for run-move step`() {
        // After 2nd deal, pile 3 = [c10, b2]. The foundation step (b2 → bastoni) exposes c10,
        // which then becomes the landing spot for the [c7, c5] run move.
        val deck = TiramisuDeckSetup.tutorialDeck()
        assertEquals("c10", deck[3])
        assertEquals("b2",  deck[7])
        // Same suit between deck[3] (c10) and the c5,c7 run built earlier so the run can land
        assertEquals("c10 e i due card del run devono condividere il seme (coppe)",
                     deck[3][0], deck[4][0])
    }
